var cgh = function (sites, callback) {
    ...
    return function cgh(req, res, next) {
        ...
        if (Object.keys(self.sites).indexOf(req.url) === -1 || req.header('x-github-event') !== 'push') return next();
        var payload;
        payload = typeof req.body.payload === 'object' ? req.body.payload : JSON.parse(req.body.payload);
